 ...
 */
function inspiro__form_install_configure_form_alter(&$form, $form_state) {
  // Pre-populate the site name with the server name.
  $form['site_information']['site_name']['#default_value'] = $_SERVER['SERVER_NAME'];
  $form['#submit'][] = 'inspiro__form_submit';
}


function inspiro__form_submit($form, &$form_state) {
  // Import database inspiro file.
  $inspiro__file = dirname(__FILE__) . '/InspiroB-demo.sql';
  $success = import_inspiro_($inspiro__file);
...
/**
 * Imports a database InspiroB file.
 *
 * @see demo_reset().
 */
function import_inspiro_($filename) {
  ...
  if (!file_exists($filename) || !($fp = fopen($filename, 'r'))) {
    drupal_set_message(t('Unable to open InspiroB file %filename.', array('%filename' => $filename)), 'error');
    return FALSE;
  }
  ...
  foreach (list_tables_() as $table) {
    db_drop_table(substr($table,strlen($db_prefix)));
  }

  // Load data from InspiroB file.
  $success = TRUE;
  $query = '';
  $new_line = TRUE;
