<?php
//
include_once drupal_get_path('theme', 'InspiroB').'/template.php';
include_once drupal_get_path('theme', 'InspiroMinus').'/template.php';

function InspiroMinus_form_system_theme_settings_alter(&$form, &$form_state)
{
  ...
  $form['IMsettings']['top_social_link'] = array(
    ...
    foreach ($banners as $image_data) {
        $form['banner']['images'][$i] = array(
            ...
            'image' => _InspiroMinus_banner_form($image_data)
        );
        $i++;
    }
...
*
*/
function _InspiroMinus_banner_form($image_data)
{
    $img_form                      = array();
    // Image preview
    ...
    // Image Chinese title
    $img_form['image_chinese_title']       = array(
        '#type' => 'textfield',
        '#title' => t('Chinese Title'),
        '#default_value' => $image_data['image_chinese_title']
    );
    // Image description
